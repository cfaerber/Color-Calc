# This Makefile is for the Color::Calc extension to perl.
#
...
#     ABSTRACT_FROM => q[Calc.pm]
#     AUTHOR => q[Claus Färber <perl@faerber.muc.de>]
#     NAME => q[Color::Calc]
#     PREREQ_PM => { Params::Validate=>q[0.75], Graphics::ColorNames::SVG=>q[0], Graphics::ColorNames=>q[0.32] }
...
VERSION_SYM = 0_29_0002
DEFINE_VERSION = -D$(VERSION_MACRO)=\"$(VERSION)\"
XS_VERSION = 0.29_0002
...
INST_LIBDIR      = $(INST_LIB)/Color
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Color
...
PM_TO_BLIB = Calc/WWW.pm \
	$(INST_LIB)/Color/Calc/WWW.pm \
	Calc/tuple.pm \
	$(INST_LIB)/Color/Calc/tuple.pm \
	Calc/hex.pm \
	$(INST_LIB)/Color/Calc/hex.pm \
	Calc/object.pm \
	$(INST_LIB)/Color/Calc/object.pm \
	Calc/html.pm \
	$(INST_LIB)/Color/Calc/html.pm \
	Calc.pm \
	$(INST_LIB)/Color/Calc.pm
...
DISTNAME = Color-Calc
DISTVNAME = Color-Calc-0.29_0002
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW)\
	  Calc.pm $(INST_MAN3DIR)/Color::Calc.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(INST_LIB)/Color/Calc/html.pm $(INST_LIB)/Color/Calc/tuple.pm $(INST_LIB)/Color/Calc/hex.pm $(MAKEFILE_OLD) $(INST_LIB)/Color/Calc.pm $(INST_LIB)/Color/Calc/WWW.pm $(INST_LIB)/Color/Calc/object.pm
	$(RM_F) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         Color-Calc' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.29_0002' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: Calc.pm' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    Graphics::ColorNames:          0.32' >> META.yml
	$(NOECHO) $(ECHO) '    Graphics::ColorNames::SVG:     0' >> META.yml
	$(NOECHO) $(ECHO) '    Params::Validate:              0.75' >> META.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,29_0002,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Simple calculations with RGB colors.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Claus Färber &lt;perl@faerber.muc.de&gt;</AUTHOR>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Params-Validate" VERSION="0,75,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  Calc/WWW.pm $(INST_LIB)/Color/Calc/WWW.pm \
	  Calc/tuple.pm $(INST_LIB)/Color/Calc/tuple.pm \
	  Calc/hex.pm $(INST_LIB)/Color/Calc/hex.pm \
	  Calc/object.pm $(INST_LIB)/Color/Calc/object.pm \
	  Calc/html.pm $(INST_LIB)/Color/Calc/html.pm \
	  Calc.pm $(INST_LIB)/Color/Calc.pm 
	$(NOECHO) $(TOUCH) $@
